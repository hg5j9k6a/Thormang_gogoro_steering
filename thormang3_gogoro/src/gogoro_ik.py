...
class Gogoro_IK:
    ...
    def get_IK_pos(self,data):
        ...
        if arm == "left_arm":
            current_pose = fwd_kinematics(left_currents_theta,"left_arm")
        elif arm == "right_arm":
    ...
    def IK_func(self,data):
        ...
        if arm[0] == "left_arm":
            ...
            joint_name = np.hstack((left_arm_joint,right_arm_joint))
        ...
        joint_speed  =  [ 10 for _ in joint_name ]
        joint_torque =  [ 10 for _ in joint_name ]
        
        joint.velocity  =   [ np.interp(joint_speed[i], [0, 100], [0, 3.546830068]) for i in range(len(joint_name))]
        # joint.effort    =   [ np.interp(joint_torque[i], [0, 100], [0, 32.090233596]) for i in range(len(joint_name)) ]
        joint.effort    =   [ 0 for i in range(len(joint_name)) ]
